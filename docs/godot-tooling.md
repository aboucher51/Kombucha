...
Read `PROJECT_MAP.md` before grepping for which scene carries a script,
who emits or listens to a signal, what a `class_name` is used by, or
whether a node path exists. It is generated by `godot-map` (its own repo,
`~/godot-projects/godot-map`; `uv tool install --editable` it once per
machine) and committed, with the full graph in `.godot-map/graph.json`:

```bash
godot-map .                          # regenerate after adding or moving scenes or scripts
godot-map . --who Enemy --depth 2    # what references a path, class_name or autoload
godot-map . --paths scripts/hud.gd   # every $Node / %Unique path in a script, checked against its scenes
```

Without it Claude reads twenty `.tscn` files to find one connection, or
guesses a node path that does not exist. `check.sh` fails on a stale map
when the tool is installed and prints `--` when it is not, so a map that
fell behind the scenes never reaches CI; a project with no map yet gets a
note, not a failure — run `godot-map .` once and commit both files. The
hook in `tools/seeds/claude-hooks.json` regenerates the map after every
`.gd`/`.tscn` edit; merge it into the project's `.claude/settings.json`
(settings are not synced: they carry each project's permissions).
